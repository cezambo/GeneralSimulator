...
Os onze domínios estão escritos: `W` mundo, `R` substrato, `O` objetos, `A` agente, `B` corpo, `C` cognição, `S` interação, `V` validador, `U` interface, `L` camada LLM e `X` transversal. Somados, 421 requisitos com critério de aceite.
